...
# # 在 Render 上，/tmp 默认可写，所以不需要创建文件夹
# # 你可以移除 os.makedirs(UPLOAD_FOLDER)
...
# Configure upload folder to a writable temporary directory on Render
UPLOAD_FOLDER = '/tmp'
ALLOWED_EXTENSIONS = {'xlsx', 'xls'}
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
...
@app.route("/", methods=['GET', 'POST'])
def upload_file():
    """Handles file upload and triggers data analysis."""
    if request.method == 'POST':
        if 'file' not in request.files:
            return redirect(request.url)
        file = request.files['file']
        ...
        if file and allowed_file(file.filename):
            ...
            try:
                # Run the analysis and get the report data
                analyzer = SalesAnalyzer(analysis_config)
                dashboard_data = analyzer.prepare_html_report_data()
                
                if dashboard_data:
                    data_json = json.dumps(dashboard_data, ensure_ascii=False)
                    # Render the report page with the analysis data
                    return render_template('sales_analysis_report.html', data_json=data_json)
                else:
                    return "❌ 分析失败，请检查上传文件的内容。", 500
            except Exception as e:
