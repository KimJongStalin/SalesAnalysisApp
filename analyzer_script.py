    ...
    def prepare_and_get_data(self, user_choices=None):
        ...
        for dim_key in share_dimensions_to_run:
          if dim_key not in cols:
            print(f"警告: 维度 '{dim_key}' 在 columns 配置中未定义，跳过市场份额分析。")
            continue
          dim_col_name = cols[dim_key]
          if dim_col_name not in self.df.columns:
            print(f"警告: 列名 '{dim_col_name}' 在Excel中不存在，跳过市场份额分析。")
            continue
          ...
          for p_type in product_types:
            ...
            if df_filtered[dim_col_name].nunique() > top_n:
              total_sales = df_filtered.groupby(dim_col_name)[sales_col].sum()
            ...
            data_pct_sorted = data_sorted.div(safe_data_sum, axis=0) * 100
            datasets = [{"label": str(col), "data": data_pct_sorted[col].round(1).tolist(), "absoluteData": data_sorted[col].round(0).tolist()} for col in data_sorted.columns]
            share_data[dim_key][p_type] = { "labels": data_pct_sorted.index.to_period('Q').strftime(time_format).tolist(), "datasets": datasets }
