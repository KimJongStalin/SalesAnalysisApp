...
analysis_config = {
    ...
    "columns": {
        # 在这里定义您Excel列名与程序内部键的映射
        "date": "Date", "sales": "Sales", "type": "类型", "brand": "Brand",
        "packsize": "产品支数", "pricerange": "ASP区间", "tiptype": "tiptype", "asin": "ASIN",
        "first_listed_date": "上架时间",
        # 【可扩展】如果未来您的Excel中有'颜色'列，在这里添加:
        # "color": "颜色", 
    },
    
    # 【【【 核心修改：动态分析维度配置 】】】
    "dynamic_analysis_dimensions": {
        # 定义所有希望进行单一维度分析的列
        "single_dimensions": [
            {"key": "brand", "display_name": "按品牌"},
            {"key": "packsize", "display_name": "按数量"},
            {"key": "pricerange", "display_name": "按价格段"},
            {"key": "tiptype", "display_name": "按笔头"},
            # 【可扩展】如果上面 columns 中添加了 color, 在这里加入即可自动分析
            # {"key": "color", "display_name": "按颜色"},
        ],
        # 定义所有希望进行交叉维度分析的列组合
        "cross_dimensions": [
            {"keys": ["tiptype", "packsize"], "display_name": "按笔头 & 数量"},
            # 【可扩展】未来您可以增加新的交叉维度
            # {"keys": ["brand", "pricerange"], "display_name": "按品牌 & 价格段"},
        ]
    },
    "time_events": {
        "Prime Day": "07-16", "Back to School": "08-15", "Prime Fall Event": "10-10",
...
class SalesAnalyzer:
    ...
    # 【【【 这是需要粘贴的新代码块 】】】
        ...
        # 解析单维度
        if "single_dimensions" in dynamic_dims_config:
            for dim_info in dynamic_dims_config["single_dimensions"]:
        ...
        # 解析交叉维度
        if "cross_dimensions" in dynamic_dims_config:
            for dim_info in dynamic_dims_config["cross_dimensions"]:
                all_dimensions_to_analyze.append({
        ...
        for dim_config in all_dimensions_to_analyze:
            ...
            group_by_keys = dim_config["group_by_keys"]
            ...
            # --- 动态计算增长表 ---
            print(f"--- 正在动态计算增长表: {display_name} ---")
            table_data[dim_key] = {}
            for p_type in product_types:
                try:
                    df_filtered = self.df if p_type == "Overall" else self.df[self.df[type_col] == p_type]
                    ...
                    sales = df_filtered.groupby(group_by_cols + [pd.Grouper(key=date_col, freq='Q')])[sales_col].sum().unstack(date_col).fillna(0)
                    if sales.empty: continue
                    ...
                    if dim_key == 'brand' and len(sales) > 20:
                        ...
                        other_row = pd.DataFrame(other_sales).T
                        # 兼容多索引和单索引
                        other_row.index = [('其他 (Others)',)] if is_multi_dim and len(group_by_cols) > 1 else ['其他 (Others)']
                        sales = pd.concat([top_sales, other_row])
            ...
            # --- 动态计算战略气泡图 ---
            print(f"--- 正在动态计算战略定位矩阵: {display_name} ---")
            for p_type in product_types:
                if p_type not in strategic_positioning_data:
                    strategic_positioning_data[p_type] = {}
                
                df_slice_by_type = self.df if p_type == "Overall" else self.df[self.df[type_col] == p_type]
                if df_slice_by_type.empty: 
                    strategic_positioning_data[p_type][dim_key] = []
                    continue
                
                try:
                    quarterly_total_sales_slice = df_slice_by_type.groupby(pd.Grouper(key=date_col, freq='Q'))[sales_col].sum()
                    sales_by_dim = df_slice_by_type.groupby(group_by_cols + [pd.Grouper(key=date_col, freq='Q')])[sales_col].sum().unstack(level=date_col).fillna(0)

                    if len(sales_by_dim.columns) < 8: 
                        strategic_positioning_data[p_type][dim_key] = []
                        continue
        ...
        return {
            ...
            "dynamic_analysis_dimensions": dynamic_dims_config, # <--- 添加这一行
            "salesForecast": forecast_data,
            "quarterlyYoY": quarterly_yoy_data, 
            "shareAnalysis": share_data, 
            "growthTables": table_data, # <--- 使用新的 table_data
            "paretoAnalysis": pareto_data,
            "starProductAnalysis": star_product_analysis,
            "structuralKpis": structural_kpis,
            "strategicPositioning": strategic_positioning_data # <--- 使用新的 strategic_positioning_data
        }
