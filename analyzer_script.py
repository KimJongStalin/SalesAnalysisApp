...
#     "html_report": { 
#         "enabled": True, 
#         "template_path": "sales_analysis_report.html", 
#     },
#     "columns": {
...
#     },
#     "time_events": {
#         "Prime Day": "07-16", "Back to School": "08-15", "Prime Fall Event": "10-10",
#         "Black Friday": "11-29", "Christmas": "12-25", "Valentine's Day": "02-14",
#         "Mother's Day": "05-11"
#     },
# }
...
#         self.config = config
#         self.df = None
#         self.dashboard_data = {} # 用于存储最终给报告的数据
...
#                     "historical": [{'x': r['ds'].strftime('%Y-%m-%d'), 'y': r['y']} for _, r in prophet_df.iterrows()],
#                     "forecast": [{'x': r['ds'].strftime('%Y-%m-%d'), 'y': round(r['yhat'], 0), 'y_lower': round(r['yhat_lower'], 0), 'y_upper': round(r['yhat_upper'], 0)} for _, r in forecast.iterrows()],
#                     "moving_average": [{'x': date.strftime('%Y-%m-%d'), 'y': round(value, 0)} for date, value in monthly_sales.rolling(window=3).mean().dropna().items()],
#                     "trend_line": [{'x': r['ds'].strftime('%Y-%m-%d'), 'y': round(r['trend'], 0)} for _, r in forecast.iterrows()]
#                 }
...
#         """
#         template_path = self.config['html_report'].get('template_path', 'sales_analysis_report.html')
#         try:
#             print(f"\n正在从模板 '{template_path}' 生成网页报告...")
#             with open(template_path, 'r', encoding='utf-8') as f:
#                 template_str = f.read()
            
#             data_json_str = json.dumps(self.dashboard_data, indent=4, ensure_ascii=False, default=str)
...
# # 用于独立测试此文件的部分 (Flask 服务器不会运行这里)
# if __name__ == '__main__':
#     # 这是一个示例，用于您想单独测试这个文件时使用
#     print("--- 正在独立测试 SalesAnalyzer ---")
    
#     # 必须提供一个有效的 excel 文件名才能成功测试
#     test_config = analysis_config.copy()
#     test_config['input_filepath'] = 'liquid highlighters.xlsx' 
    
#     analyzer = SalesAnalyzer(config=test_config)
#     if analyzer.run_analysis():
#         analyzer.export_to_html(output_path='test_report.html')
#         print("--- 独立测试成功 ---")
...
# 文件名: analyzer_script.py (最终完整版 - 忠于原始设计)
...
analysis_config = {
    ...
    "header_mappings": { 
        "brand": "Brand", "packsize": "PackSize", "pricerange": "PriceRange", 
        "tiptype": "TipType", "tiptype_packsize": "TipType & PackSize" 
    },
    "time_events": {
        "Prime Day": "07-16", "Back to School": "08-15", "Prime Fall Event": "10-10",
        "Black Friday": "11-29", "Christmas": "12-25", "Valentine's Day": "02-14",
...
class SalesAnalyzer:
    ...
    def load_and_preprocess_data(self) -> bool:
        ...
        try:
            ...
            print(f"❌ 读取Excel文件时出错: {e}"); return False

        date_col = cols.get('date', 'Date')
        if date_col not in self.df.columns:
            print(f"❌ 错误: 在Excel中找不到指定的日期列 '{date_col}'"); return False
        ...
        if sales_col not in self.df.columns:
            print(f"❌ 错误: 在Excel中找不到指定的销售列 '{sales_col}'"); return False

        self.df[sales_col] = pd.to_numeric(self.df[sales_col], errors='coerce').fillna(0)
        self.df.sort_values(by=date_col, inplace=True)

        if self.df.empty:
            print("⚠️ 警告: 清理后没有剩下有效的数据。"); return False
    ...
    def prepare_and_get_data(self, user_choices=None):
        ...
        pareto_data, star_product_analysis, structural_kpis = {}, {}, {}
        strategic_positioning_data = {}
        ...
        for p_type in product_types:
            df_filtered = self.df if p_type == "Overall" else self.df[self.df[type_col] == p_type]
            monthly_sales = df_filtered.set_index(date_col)[sales_col].resample('ME').sum()
            if len(monthly_sales) >= 12:
                prophet_df = monthly_sales.reset_index(); prophet_df.columns = ['ds', 'y']
                model = Prophet(yearly_seasonality=True, weekly_seasonality=False, daily_seasonality=False, growth='linear')
                model.fit(prophet_df)
                future = model.make_future_dataframe(periods=12, freq='ME')
                forecast = model.predict(future)
                forecast_data[p_type] = { "historical": [{'x': r['ds'].strftime('%Y-%m-%d'), 'y': r['y']} for _, r in prophet_df.iterrows()], "forecast": [{'x': r['ds'].strftime('%Y-%m-%d'), 'y': round(r['yhat'], 0), 'y_lower': round(r['yhat_lower'], 0), 'y_upper': round(r['yhat_upper'], 0)} for _, r in forecast.iterrows()], "moving_average": [{'x': date.strftime('%Y-%m-%d'), 'y': round(value, 0)} for date, value in monthly_sales.rolling(window=3).mean().dropna().items()], "trend_line": [{'x': r['ds'].strftime('%Y-%m-%d'), 'y': round(r['trend'], 0)} for _, r in forecast.iterrows()] }

        print("--- 正在计算季度同比增长 ---")
        sales_wide_q = self.df.groupby([pd.Grouper(key=date_col, freq='QE'), type_col])[sales_col].sum().unstack(type_col).fillna(0)
        yoy_wide_q = sales_wide_q.pct_change(periods=4) * 100
        quarterly_yoy_data = {"labels": sales_wide_q.index.to_period('Q').strftime('%YQ%q').tolist(), "sales_datasets": [{"label": str(col), "data": sales_wide_q[col].round(0).tolist()} for col in sales_wide_q.columns], "yoy_datasets": [{"label": str(col) + " YoY", "data": yoy_wide_q[col].where(pd.notna(yoy_wide_q[col]), None).round(1).tolist()} for col in yoy_wide_q.columns]}
        ...
        for dim in share_dimensions:
            ...
            for p_type in product_types:
                ...
                data_pct_sorted = data_sorted.div(safe_data_sum, axis=0) * 100
                datasets = [{"label": str(col), "data": data_pct_sorted[col].round(1).tolist(), "absoluteData": data_sorted[col].round(0).tolist()} for col in data_sorted.columns]
                share_data[dim][p_type] = { "labels": data_pct_sorted.index.to_period(freq.replace('E','')).strftime(time_format).tolist(), "datasets": datasets }
        ...
        for key, dim_names in table_dimensions.items():
            ...
            for p_type in product_types:
                df_filtered = self.df if p_type == "Overall" else self.df[self.df[type_col] == p_type]
                if df_filtered.empty: continue
                sales = df_filtered.groupby(dim_cols + [pd.Grouper(key=date_col, freq='QE')])[sales_col].sum().unstack(date_col).fillna(0)
                if sales.empty: continue
                if len(sales.columns) > 0:
                    sales = sales.sort_values(by=sales.columns[-1], ascending=False)
                if key == 'brand' and len(sales) > 20:
                    top_sales = sales.head(20)
                    other_sales = sales.iloc[20:].sum()
                    other_row = pd.DataFrame(other_sales).T
                ...
                if '其他 (Others)' in sales.index:
                    ...
                    sales = pd.concat([sales, other_row])
                display_map = self.config.get("header_mappings", {})
                display_names = [display_map.get(name, name.capitalize()) for name in dim_names]
                display_header = " & ".join(display_names)
                headers = [display_header] + [q.to_period('Q').strftime('%YQ%q') for q in sales.columns]
                rows = []
                for index, row_data in sales.iterrows():
                    is_multi_index = isinstance(index, tuple)
                    row_content = [{'type': 'label', 'value': ' & '.join(map(str, index)) if is_multi_index else str(index)}]
                    for i, quarter in enumerate(sales.columns):
                        sale_val = row_data.get(quarter, 0)
                        yoy_val, yoy_status = None, 'neutral'
                        if sale_val > 0 and i >= 4:
                            prior_val = row_data.get(sales.columns[i-4], 0)
                            if prior_val > 0:
                                yoy = (sale_val / prior_val) - 1
                                yoy_val, yoy_status = f"{yoy:.1%}", 'positive' if yoy > 0 else 'negative'
                            else:
                                yoy_val, yoy_status = "New", 'positive'
                        sale_str = f"{(sale_val / 10000.0):.2f}万" if sale_val != 0 else "-"
                        row_content.append({'type': 'data', 'value': sale_str, 'yoy': yoy_val, 'yoy_status': yoy_status})
                    rows.append(row_content)
                table_data[key][p_type] = {"headers": headers, "rows": rows}
        ...
        for p_type in product_types:
            ...
            for dim_key, dim_col_config in dims_to_analyze.items():
                is_multi_dim = isinstance(dim_col_config, tuple)
                group_cols = list(dim_col_config) if is_multi_dim else [dim_col_config]
                if any(c is None for c in group_cols) or not all(c in df_slice_by_type.columns for c in group_cols): continue
                if df_slice_by_type.empty or len(df_slice_by_type[date_col].dt.to_period('Q').unique()) < 8: continue
                try:
                    quarterly_total_sales_slice = df_slice_by_type.groupby(pd.Grouper(key=date_col, freq='QE'))[sales_col].sum()
                    sales_by_dim = df_slice_by_type.groupby(group_cols + [pd.Grouper(key=date_col, freq='QE')])[sales_col].sum().unstack(level=date_col).fillna(0)
                    if len(sales_by_dim.columns) < 8: continue
                    recent_quarters, prior_quarters = sales_by_dim.columns[-4:], sales_by_dim.columns[-8:-4]
                    if not all(q in quarterly_total_sales_slice.index for q in recent_quarters) or not all(q in quarterly_total_sales_slice.index for q in prior_quarters): continue
                    yearly_sales = sales_by_dim[recent_quarters].sum(axis=1)
                    prior_yearly_sales = sales_by_dim[prior_quarters].sum(axis=1)
                    total_market_yearly_sales = quarterly_total_sales_slice.loc[recent_quarters].sum()
                    market_share = (yearly_sales / total_market_yearly_sales) * 100 if total_market_yearly_sales > 0 else 0
                    ...
                    for dim_value_tuple in sales_by_dim.index:
                        label_str = str(dim_value_tuple) if not is_multi_dim else ' & '.join(map(str, dim_value_tuple))
                        bubble_data.append({"label": label_str, "x": round(market_share.loc[dim_value_tuple], 1), "y": round(yoy_growth.loc[dim_value_tuple], 1), "r": int(yearly_sales.loc[dim_value_tuple])})
                    strategic_positioning_data[p_type][dim_key] = bubble_data
                except Exception as e:
                    print(f"❌ 为 '{p_type}' 的 '{dim_key}' 维度生成气泡图时发生错误: {e}")
                    strategic_positioning_data[p_type][dim_key] = []
        ...
        for p_type in product_types:
            ...
            quarterly_sales = df_filtered.groupby([asin_col, 'quarter'])[sales_col].sum().unstack().fillna(0)
            star_product_analysis[p_type] = {}
            if len(quarterly_sales.columns) < 4: continue
            product_quadrant_paths = {}
            for i in range(3, len(quarterly_sales.columns)):
                current_quarter = quarterly_sales.columns[i]
                recent_quarters = quarterly_sales.columns[i-3 : i+1]
                ...
                for index, p in enumerate(points):
                    if not p['is_new']:
                        ...
                        points[index]['x'] = round(growth_score, 1)
                if mature_growth_scores:
                    mature_points_df = pd.DataFrame([p for p in points if not p['is_new'] and 'x' in p])
                    growth_p90 = np.percentile(mature_growth_scores, 90)
                    growth_p65 = np.percentile(mature_growth_scores, 65)
                    volume_p20 = mature_points_df['y'].quantile(0.20) if not mature_points_df.empty else 0
                    for p in points:
                        if p['is_new']:
                            p['x'] = round(growth_p90 if p['y'] >= volume_p20 else growth_p65, 1)
                for p in points:
                    if 'sales_series' in p: del p['sales_series']
                final_points = [p for p in points if 'x' in p]
                star_product_analysis[p_type][str(current_quarter)] = {"points": final_points}
        ...
        for p_type in product_types:
            ...
            df_filtered['quarter'] = df_filtered[date_col].dt.to_period('Q')
            all_quarters = sorted(df_filtered['quarter'].unique())
            kpi_results = {'labels': [], 'top20_contrib': [], 'top50_contrib': [], 'new_sales_pct': []}
            first_listed_date_col = cols.get('first_listed_date')
            for i, quarter in enumerate(all_quarters):
                kpi_results['labels'].append(str(quarter))
                quarter_df = df_filtered[df_filtered['quarter'] == quarter]
                total_sales = quarter_df[sales_col].sum()
                if total_sales > 0:
                    sales_by_asin = quarter_df.groupby(asin_col)[sales_col].sum().sort_values(ascending=False)
                    kpi_results['top20_contrib'].append(round((sales_by_asin.head(20).sum() / total_sales) * 100, 1))
                    kpi_results['top50_contrib'].append(round((sales_by_asin.head(50).sum() / total_sales) * 100, 1))
                    if first_listed_date_col and first_listed_date_col in quarter_df.columns:
                        new_asins = quarter_df[(quarter_df[first_listed_date_col].notna()) & ((quarter.end_time - quarter_df[first_listed_date_col]).dt.days <= 90)][asin_col].unique()
                        new_sales = quarter_df[quarter_df[asin_col].isin(new_asins)][sales_col].sum()
                        kpi_results['new_sales_pct'].append(round((new_sales / total_sales) * 100, 1))
                    else:
                        kpi_results['new_sales_pct'].append(0)
                else:
                    kpi_results['top20_contrib'].append(0)
                    kpi_results['top50_contrib'].append(0)
                    kpi_results['new_sales_pct'].append(0)
        ...
        print("\n✅ 核心分析流程全部完成！")
        return {
            "product_types": product_types, "time_events": dynamic_time_events,
            "salesForecast": forecast_data, "quarterlyYoY": quarterly_yoy_data,
            "shareAnalysis": share_data, "growthTables": table_data,
            "strategicPositioning": strategic_positioning_data,
            "paretoAnalysis": pareto_data, "starProductAnalysis": star_product_analysis,
            "structuralKpis": structural_kpis
        }
        # 将所有计算结果存入实例变量
        # self.dashboard_data = {
